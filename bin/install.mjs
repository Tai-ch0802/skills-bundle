  ...
  en: {
    ...
    messages: {
      ...
      skillDescriptions: {
        ...
        'rust-pro': 'Master modern Rust (2024 edition) with async patterns, advanced type system features, and production-ready systems programming. Expert in the current Rust ecosystem including Tokio, axum, and modern crates. Use PROACTIVELY for Rust development, performance optimization, or systems programming.',
  ...
  'zh-TW': {
    ...
    messages: {
      ...
      skillDescriptions: {
        ...
        // Antigravity Kit 技能
        'api-patterns': 'API 設計原則與決策。REST vs GraphQL vs tRPC 選擇、回應格式、版本控制、分頁。',
        'app-builder': '主要應用程式建構協調器。從自然語言需求建立全端應用程式。決定專案類型、選擇技術堆疊、協調代理。',
        architecture: '架構決策框架與 ADR 文件',
        'bash-linux': 'Bash/Linux 終端模式。關鍵指令、管道、錯誤處理、腳本編寫。在 macOS 或 Linux 系統上工作時使用。',
        'behavioral-modes': 'AI 操作模式（腦力激盪、實作、除錯、審查、教學、發布、協調）。用於根據任務類型調整行為。',
        brainstorming: '蘇格拉底式提問協議，用於複雜需求與新功能',
        'code-quality': '務實的編碼標準與程式碼審查指南（合併自 clean-code + code-review-checklist）',
        'database-design': '資料庫設計原則與決策。Schema 設計、索引策略、ORM 選擇、無伺服器資料庫。',
        'deployment-procedures': '生產環境部署原則與決策。安全的部署流程、回滾策略和驗證。教授思考方式，而非腳本。',
        'documentation-templates': '文件範本與結構指南。README、API 文件、程式碼註解以及對 AI 友善的文件。',
        'game-development': '遊戲開發協調器。根據專案需求路由至特定平台的技能。',
        ...
        'geo-fundamentals': '生成式引擎最佳化（GEO），針對 AI 搜尋引擎（ChatGPT、Claude、Perplexity）。',
        humanizer: '移除 AI 寫作模式 — 膨脹象徵主義、AI 詞彙、破折號濫用、模糊歸因',
        'i18n-localization': '國際化與本地化模式。偵測寫死的字串、管理翻譯、本地化檔案、RTL 支援。',
        'intelligent-routing': '自動代理選擇與智慧任務路由。分析使用者需求並自動選擇最佳的專家代理，無需使用者明確指定。',
        'lint-and-validate': '自動品質控制、程式碼檢查與靜態分析程序。在每次修改程式碼後使用，以確保語法正確性和專案標準。',
        'mobile-design': 'iOS 和 Android 應用程式的行動優先設計思維與決策。觸控互動、效能模式、平台慣例。教授原則，而非固定值。用於建構 React Native、Flutter 或原生行動應用程式時。',
        'nextjs-react-expert': 'React 與 Next.js 效能最佳化（來自 Vercel 工程團隊）。在建構 React 元件、最佳化效能、消除瀑布流、減少套件大小、審查程式碼效能問題或實作伺服器端/客戶端最佳化時使用。',
        'nodejs-best-practices': 'Node.js 開發原則與決策。框架選擇、非同步模式、安全性與架構。教授思考方式，而非複製貼上。',
        'parallel-agents': '多代理協調模式。當多個獨立任務可以由不同領域專家執行，或全面分析需要多種視角時使用。',
        'performance-profiling': '效能分析原則。量測、分析與最佳化技術。',
        'plan-writing': '具有清晰分解、相依性和驗證準則的結構化任務規劃。在實作功能、重構或任何多步驟工作時使用。',
        'powershell-windows': 'PowerShell Windows 模式。關鍵陷阱、運算子語法、錯誤處理。',
        'python-patterns': 'Python 開發原則與決策。框架選擇、非同步模式、型別提示、專案結構。教授思考方式，而非複製貼上。',
        'red-team-tactics': '基於 MITRE ATT&CK 的紅隊戰術原則。攻擊階段、規避偵測、報告。',
        'rust-pro': '精通現代 Rust (2024 版本)，包含非同步模式、進階型別系統功能，以及可投入生產的系統程式設計。熟悉目前 Rust 生態系，包含 Tokio、axum 與現代 crates。積極用於 Rust 開發、效能最佳化或系統程式設計。',
        'seo-fundamentals': 'SEO 基礎、E-E-A-T、Core Web Vitals 和 Google 演算法原則。',
        'server-management': '伺服器管理原則與決策。程序管理、監控策略與擴展決策。教授思考方式，而非指令。',
        'skill-vetter': '安全優先的技能審查 — 紅色旗標、權限範圍、可疑模式偵測',
        'systematic-debugging': '四階段系統化除錯方法論，包含根因分析和基於證據的驗證。在除錯複雜問題時使用。',
        'tailwind-patterns': 'Tailwind CSS v4 原則。CSS 優先配置、容器查詢、現代模式、設計 Token 架構。',
        'testing-mastery': '統一測試技能 — TDD、單元/整合模式、E2E/Playwright（合併自 tdd-workflow + testing-patterns + webapp-testing）',
        'vulnerability-scanner': '進階漏洞分析原則。OWASP 2025、軟體供應鏈安全、攻擊面映射、風險優先順序評估。',
        ...
        'web-design-guidelines': '檢查 UI 程式碼是否符合 Web 介面指南。當被要求「審查我的 UI」、「檢查無障礙設計」、「稽核設計」、「審查 UX」或「檢查我的網站是否符合最佳實務」時使用。',
