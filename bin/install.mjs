  ...
  'zh-TW': {
    ...
    messages: {
      ...
      skillDescriptions: {
        ...
        'claude-api': '使用 Claude API / Anthropic SDK 建構、除錯和最佳化應用程式。使用此技能建構的應用程式應包含提示快取 (prompt caching)。觸發時機：程式碼匯入 anthropic/@anthropic-ai/sdk；使用者要求使用 Claude API、Anthropic SDKs 或 Managed Agents (/v1/agents, /v1/sessions, /v1/environments)。不要觸發時機：程式碼匯入 `openai`/其他 AI SDK、一般程式設計，或機器學習/資料科學任務。',
