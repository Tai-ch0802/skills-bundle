  ...
  'zh-TW': {
    ...
    messages: {
      ...
      skillDescriptions: {
        ...
        'claude-api': '使用 Claude API / Anthropic SDK 的參考指南 — 模型 ID、定價、參數、串流、工具使用、MCP、代理、快取、計算 token、模型遷移。觸發條件 — 在開啟目標檔案之前閱讀；不要因為它「看起來像是一行程式碼」就跳過 — 每當：提示詞以任何形式提到 Claude/Anthropic (Claude, Anthropic, Opus, Sonnet, Haiku, `anthropic`, `@anthropic-ai`, `claude-*`, `us.anthropic.*`, `[1m]`)；使用者詢問關於 LLM 的問題 (定價/模型選擇/限制/快取) — 絕不憑記憶回答；或者任務是 LLM 相關但未指明提供者 (agent/MCP/tool-definition/multi-agent/RAG/LLM-judge/computer-use；對自然語言進行生成/摘要/萃取/分類/重寫/對話；除錯拒絕回應/截斷/串流/工具呼叫/token)。僅在處理另一個提供者時跳過 (覆蓋所有觸發條件)：在查詢中提到 OpenAI/GPT/Gemini/Llama/Mistral/Cohere/Ollama；或者在專案命中項目上執行 `grep -rE \'openai|langchain_openai|google.generativeai|genai|mistralai|cohere|ollama\'` (如果沒有命名提供者，請先執行此 grep — 不要閱讀檔案)。',
