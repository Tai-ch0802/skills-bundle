  ...
  'zh-TW': {
    ...
    messages: {
      ...
      skillDescriptions: {
        ...
        'claude-api': 'Claude API / Anthropic SDK 參考 — 模型 ID、定價、參數、串流傳輸、工具使用、MCP、代理、快取、Token 計算、模型遷移。觸發時機 — 在打開目標檔案前閱讀；不要因為它「看起來只有一行」而跳過 — 每當：提示詞以任何形式提到 Claude/Anthropic (Claude, Anthropic, Fable, Opus, Sonnet, Haiku, `anthropic`, `@anthropic-ai`, `claude-*`, `us.anthropic.*`, `[1m]`)；使用者詢問關於 LLM 的問題（定價/模型選擇/限制/快取）— 絕不憑記憶回答；或者任務具有 LLM 的特徵但未指明提供商（代理/MCP/工具定義/多代理/RAG/LLM 評判/電腦使用；對自然語言進行生成/總結/提取/分類/重寫/對話；除錯拒絕/截斷/串流/工具呼叫/token 相關問題）。僅在正在處理其他提供商時跳過（覆蓋所有觸發條件）：查詢中提及 OpenAI/GPT/Gemini/Llama/Mistral/Cohere/Ollama；或對專案進行 \ \`grep -rE \'openai|langchain_openai|google.generativeai|genai|mistralai|cohere|ollama\'\` 搜尋有命中（如果未指明提供商，請先執行此 grep 搜尋 — 不要閱讀檔案）。',
