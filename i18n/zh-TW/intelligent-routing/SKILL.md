...
description: 自動代理選擇與智慧任務路由。分析使用者需求並自動選擇最佳的專家代理，無需使用者明確指定。
version: 1.0.0
---
...
```text
使用者：「建立一個暗色模式切換按鈕」
預期：自動調用 frontend-specialist
驗證：回應顯示「使用 @frontend-specialist」
```
...
- 分析每個請求多耗約 50–100 tokens
- 取捨：較高的準確度換來些微額外成本
- 整體上**節省** tokens，因為減少了來回往返
...
```markdown
💡 **小提示**：本系統已設定為自動選擇專家代理。我會為你的任務挑選最適合的專家。如果你偏好，仍可以用 `@agent-name` 明確指定代理。
```
...
**下一步**：將此技能整合進 GEMINI.md 的 TIER 0 規則。
