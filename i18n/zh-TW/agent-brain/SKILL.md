...
> **注意**：Session 結束時**不會**自動同步至 pCloud。請使用 `/sync-brain` 將變更推送至雲端。
...
Agent Brain 附帶三個全域 workflow，可在 bootstrap 過程中安裝至 `~/.agent/workflows/`（也可稍後手動透過 `install-workflows.sh` 安裝）：

| Workflow | Slash 指令 | 用途 |
|----------|-----------|------|
| `save-brain` | `/save-brain` | 沖刷 session 記憶 → 更新 MEMORY/USER/projects → 建立索引（僅限本地） |
| `sync-brain` | `/sync-brain` | 基於 SHA 增量傳輸的 pCloud 雙向同步（含衝突解決） |
| `load-brain` | `/load-brain` | 載入 MEMORY.md、USER.md、今日/昨日 session → 載入專案上下文（僅限本地） |
