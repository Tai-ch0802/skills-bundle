...
- `gemini-2.5-flash-native-audio-preview-12-2025` — 原生音訊輸出、情感對話、主動音訊、思考。128k 上下文視窗。**這是所有 Live API 案例的建議模型。**

> [!WARNING]
> 以下 Live API 模型已**棄用**並將被關閉。請遷移至 `gemini-2.5-flash-native-audio-preview-12-2025`。
...
> [!IMPORTANT]
> 針對所有即時使用者輸入（音訊、視訊**和文字**）使用 `send_realtime_input` / `sendRealtimeInput`。僅在逐步更新對話歷史記錄（將先前的輪次附加到上下文中）時使用 `send_client_content` / `sendClientContent`，**切勿**用於傳送新的使用者訊息。
...
- [Live API 功能指南](https://ai.google.dev/gemini-api/docs/live-guide.md.txt) — 語音設定、轉錄設定、原生音訊（情感對話、主動音訊、思考）、VAD 設定、媒體解析度
- [Live API 工具使用](https://ai.google.dev/gemini-api/docs/live-tools.md.txt) — 函式呼叫（同步和非同步）、Google 搜尋背景資訊
